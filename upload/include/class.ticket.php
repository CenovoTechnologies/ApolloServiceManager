...
implements RestrictedAccess, Threadable {
    ...
    function isAssigned() {
        return ($this->isOpen() || $this->isStarted() || $this->isResolved()) && ($this->getStaffId() || $this->getTeamId());
    }
    ...
    function getAssignmentForm($source=null, $options=array()) {
        ...
        switch (strtolower($options['target'])) {
            case 'agents':
                ...
                if (!$source && ($this->isOpen() || $this->isStarted()) && $this->staff)
            ...
            case 'teams':
                ...
                if (!$source && ($this->isOpen() || $this->isStarted()) && $this->team)
    ...
    function setStatus($status, $comments='', &$errors=array(), $set_closing_agent=true) {
        ...
        switch ($status->getState()) {
            ...
            case 'open':
                ...
                if (!$this->isOpen() || !$this->isStarted() || !$this->isResolved())
                    $this->isanswered = 0;
				break;
			case 'resolved':
				if (!$this->isClosed()) {
    ...
    function setState($state, $alerts=false) {
		...
		case 'progress':
			return $this->setStatus('progress');
		case 'resolved':
			return $this->setStatus('resolved');
        case 'answered':
...
//                __('Correct any errors below and try again'));
        ...
        if ($vars['duedate']) {
            if ($this->isClosed() || $this->isResolved())
                $errors['duedate']=__('Due date can NOT be set on a closed/resolved ticket');
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function getStaffStats($staff) {
        ...
        $blocks = Ticket::objects()
            ...
            //->filter(array('status__state' => 'progress'))
    ...
    function getUserStats($user) {
        ...
        $sql='SELECT count(open.ticket_id) as open, count(closed.ticket_id) as closed , count(progress.ticket_id) as progress, count(resolved.ticket_id) as resolved '
            .' FROM '.TICKET_TABLE.' ticket '
            .' LEFT JOIN '.TICKET_TABLE.' open
                ON (open.ticket_id=ticket.ticket_id AND open.status=\'open\') '
            .' LEFT JOIN '.TICKET_TABLE.' progress
                ON (progress.ticket_id=ticket.ticket_id AND progress.status=\'progress\') '
            .' LEFT JOIN '.TICKET_TABLE.' resolved
                ON (resolved.ticket_id=ticket.ticket_id AND resolved.status=\'resolved\') '
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        if ($ticket->isOpen() || $ticket->isStarted()) {
