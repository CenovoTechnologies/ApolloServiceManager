...
require_once INCLUDE_DIR . 'class.servicesubcat.php';
require_once INCLUDE_DIR . 'class.sequence.php';

class ServiceCat extends VerySimpleModel
    implements TemplateVariable {

    static $meta = array(
        'table' => SERVICE_CAT_TABLE,
        'pk' => array('service_cat_id'),
        'ordering' => array('service_cat'),
        'joins' => array(
            'service' => array(
                'list' => false,
                'constraint' => array(
                    'service_cat_pid' => 'Service.service_id',
                ),
            ),
            'page' => array(
                'null' => true,
    ...
    static function getVarScope() {
        return array(
            'fullname' => __('Service Category full path'),
    ...
    function getFullName() {
        return self::getServiceCatName($this->getId()) ?: $this->service_cat;
    }

    static function getServiceCatName($id) {
        $names = static::getServiceCategories(false, true);
        return $names[$id];
    }

    static function getServiceCatParent($pid) {
    ...
    function getServiceSubCats() {
        return ServiceSubCat::getParentCategories($this->getId());
    }
    ...
    function getTranslateTag($subtag) {
        return _H(sprintf('service_cat.%s.%s', $subtag, $this->getId()));
    }
    ...
    function delete() {
        ...
        if ($this->getId() == $cfg->getDefaultServiceCatId())
            return false;

        if (parent::delete()) {
            self::objects()->filter(array(
                'service_cat_pid' => $this->getId()
            ))->update(array(
                'service_cat_pid' => 0
            ));
            ServiceCat::objects()->filter(array(
                'service_cat_id' => $this->getId()
            ))->delete();
            db_query('UPDATE '.TICKET_TABLE.' SET service_cat_id=0 WHERE service_cat_id='.db_input($this->getId()));
        }
    ...
    static function __create($vars, &$errors) {
        ...
        $vars['id'] = $vars['service_cat_id'];
        $serviceCat->update($vars, $errors);
        return $serviceCat;
    }

    static function getServiceCategories($publicOnly=false, $disabled=false, $localize=true) {
        global $cfg;
        ...
        if (!$names || $localize) {
            $objects = self::objects()->values_flat(
                'service_cat_id', 'service_cat_pid', 'ispublic', 'isactive', 'service_cat'
            )
                ->order_by('sort');
    ...
    static function getParentCategories($parentId, $localize=true) {
        ...
        if (!$names || $localize) {
            $objects = self::objects()->values_flat(
                'service_cat_id', 'service_cat_pid', 'ispublic', 'isactive', 'service_cat', 'notes'
            )
                ->order_by('sort');
    ...
    static function getPublicServiceCategories() {
        return self::getServiceCategories(true);
    }

    static function getAllServiceCategories($localize=false) {
        return self::getServiceCategories(false, true, $localize);
    }

    static function getLocalNameById($id) {
        $services = static::getServiceCategories(false, true);
        return $services[$id];
    }

    static function getParentById($id) {
        return Service::getServiceName($id);
    }
    ...
    static function getIdByName($name, $pid=0) {
        $list = self::objects()->filter(array(
            'service_cat'=>$name,
            'service_cat_pid'=>$pid,
        ))->values_flat('service_cat_id')->first();
    ...
    function update($vars, &$errors) {
        ...
        $vars['service_cat'] = Format::striptags(trim($vars['service_cat']));

        if (isset($this->service_cat_id) && $this->getId() != $vars['id'])
            $errors['err']=__('Internal error occurred');

        if (!$vars['service_cat'])
            $errors['service_cat']=__('Service Category name is required');
        elseif (strlen($vars['service_cat'])<5)
            $errors['service_cat']=__('Service Category name is too short. Five characters minimum');
        elseif (($tid=self::getIdByName($vars['service_cat'], $vars['service_cat_pid']))
            && (!isset($this->service_cat_id) || $tid!=$this->getId()))
            $errors['service_cat']=__('Service Category already exists');

        if ($errors)
            return false;

        $this->service_cat = $vars['service_cat'];
        $this->service_cat_pid = $vars['service_cat_pid'] ?: 0;
        ...
        if ($this->__new__) {
            if (!($rv = $this->save())) {
                $errors['err']=sprintf(__('Unable to create %s.'), __('this service category'))
                    .' '.__('Internal error occurred');
            }
        }
        elseif (!($rv = $this->save())) {
            $errors['err']=sprintf(__('Unable to update %s.'), __('this service category'))
                .' '.__('Internal error occurred');
        }
        if ($rv) {
            if (!$cfg || $cfg->getServiceCatSortMode() == 'a') {
    ...
    static function updateSortOrder() {
        ...
        if (!($names = static::getServiceCategories(false, true, false)))
            return;
        ...
        $sql = sprintf('INSERT INTO `%s` (service_id,`sort`) VALUES %s
            ...
            SERVICE_CAT_TABLE, implode(',', $update));
        db_query($sql);
    }
}

// Add fields from the standard ticket form to the ticket filterable fields
Filter::addSupportedMatches(/* @trans */ 'Service Category Template', array('serviceCatId' => 'Service Category ID'), 100);
