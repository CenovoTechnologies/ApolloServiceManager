<div id="the-lookup-form">
    ...
    if (!isset($info['lookup']) || $info['lookup'] !== false) { ?>
        <div><p id="msg_info" class="label-info"><i class="icon-info-sign"></i>&nbsp; <?php echo
                ...
                ?></p></div>
        <div class="row">
            <label style="padding: 0 15px; width:100%;">
                <input type="text" class="form-control-sm search-input" style="width:100%;"
    ...
    <div id="selected-user-info" style="display:<?php echo $user ? 'block' : 'none'; ?>;margin:5px;">
        <form method="post" class="user" action="<?php echo $info['action'] ? $info['action'] : '#users/lookup'; ?>">
            ...
            <?php }
            if ($user) { ?>
                <div><strong id="user-name"><?php echo Format::htmlchars($user->getName()->getOriginal()); ?></strong>
                </div>
                ...
                <table style="margin-top: 1em;">
                    <?php foreach ($user->getDynamicData() as $entry) { ?>
                        <tr>
                            <td colspan="2" style="border-bottom: 1px dotted black"><strong><?php
                                    echo $entry->getTitle(); ?></strong></td>
                        </tr>
                        <?php foreach ($entry->getAnswers() as $a) { ?>
                            <tr style="vertical-align:top">
                                <td style="width:30%;border-bottom: 1px dotted #ccc"><?php echo Format::htmlchars($a->getField()->get('label'));
                                    ?>:
                                </td>
    ...
    <div id="new-user-form" style="display:<?php echo $user ? 'none' : 'block'; ?>;">
        ...
        <span class="btn-group-sm pull-right">
            <button type="reset" value="<?php echo __('Reset'); ?>" class="btn btn-secondary">Reset</button>
            <button type="submit" value="<?php echo __('Add User'); ?>" class="btn btn-outline-primary">Add User</button>
        </span>
                </p>
            </form>
        <?php } else { ?>
            ...
            <p class="full-width">
        <span class="buttons pull-left">
            <input type="button" name="cancel" class="<?php echo $user ? 'cancel' : 'close' ?>"
